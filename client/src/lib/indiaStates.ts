// GST state codes for the delivery-state picker. The code decides whether an
// order is taxed CGST + SGST or IGST, so it is sent alongside the state name
// rather than re-derived on the server from free text.
//
// Kept in sync with server/src/lib/indiaStates.ts.
