...
function StaffRow({ user, roles }: { user: StaffUser; roles: StaffRole[] }) {
  ...
  return (
    <tr className="hover:bg-slate-50">
      ...
      <td className="px-4 py-3 tabular-nums text-slate-700">{user.phone}</td>
      <td className="px-4 py-3">
        <select
          value={user.role.id}
          disabled={update.isPending}
          onChange={(e) =>
            update.mutate({ id: user.id, roleId: e.target.value })
          }
      ...
      <td className="px-4 py-3 text-center">
        <input
          ...
          onChange={(e) =>
            update.mutate({ id: user.id, isActive: e.target.checked })
          }
...
function RolesTab() {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[260px_1fr]">
      ...
      {selected && (
        <RoleEditor role={selected} grouped={grouped} />
      )}
    </div>
  );
}
    ...
    <div className="rounded-card border border-slate-200 bg-white p-5">
      ...
      {role.isSuperuser ? (
        <p className="text-sm text-slate-600">
          Super-admin bypasses every permission check. New features stay
